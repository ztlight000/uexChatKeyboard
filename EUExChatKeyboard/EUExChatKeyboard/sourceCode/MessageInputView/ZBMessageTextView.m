...
- (NSUInteger)numberOfLinesOfText{
    NSUInteger lines = [ZBMessageTextView numberOfLinesForMessage:self.text];
    return lines;
}

+ (NSUInteger)maxCharactersPerLine{
    NSUInteger maxLine = ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPhone)?33:109;
    return maxLine;
}

+ (NSUInteger)numberOfLinesForMessage:(NSString *)text{
    NSUInteger length = text.length;
    NSUInteger maxLine = [ZBMessageTextView maxCharactersPerLine];
    return ( length/maxLine ) + 1;
}
#pragma end
